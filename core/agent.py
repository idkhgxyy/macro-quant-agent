...
class MacroQuantAgent:
    ...
    def _run_v2(self):
        """Full daily routine using injected services (PlanningService, ExecutionService, PersistenceService, OpsService).

        Keeps the same semantics as run_daily_routine() but delegates to
        the extracted service layer. The old methods remain untouched.
        """
    ...
    def run_daily_routine(self):
        if self._planning is not None and self._execution is not None and self._persistence is not None and self._ops is not None:
            return self._run_v2()
