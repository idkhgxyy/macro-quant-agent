...
class PersistenceServiceTests(unittest.TestCase):
    ...
    def test_save_and_load_decision_snapshot(self):
        self.svc.save_decision_snapshot("2026-05-20", {"status": "filled", "reasoning": "test"})
        loaded = self.svc.load_decision_snapshot("2026-05-20")
        self.assertIsNotNone(loaded)
        self.assertEqual(loaded.get("payload", {}).get("status"), "filled")

    def test_load_decision_snapshot_returns_none_when_missing(self):
        loaded = self.svc.load_decision_snapshot("2099-01-01")
        self.assertIsNone(loaded)

    def test_save_rag_snapshot(self):
        self.svc.save_rag_snapshot("2026-05-20", {"macro": "test macro"})
    ...
    def test_save_execution_ledger(self):
        self.svc.save_execution_ledger("2026-05-20", {"before": {"cash": 10000.0}})
    ...
    def test_save_portfolio_state(self):
        self.svc.save_portfolio_state(50000.0, {"AAPL": 100})
        self.assertTrue(os.path.exists("portfolio_state.json"))

    def test_append_metrics(self):
        self.svc.append_metrics({"date": "2026-05-20", "status": "filled", "turnover": 0.25})
