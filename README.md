...
- 侧边栏配置 API Key（DeepSeek / FMP / Alpha Vantage / AnySearch），无需手动编辑 `.env`
...
- 侧边栏直接填写 API Key（DeepSeek / FMP / Alpha Vantage / AnySearch），保存到 `.env`，无需手动编辑文件
...
- **FMP API Key**（推荐，美股主数据源，免费 250 次/天，覆盖行情+基本面+新闻+宏观）
- **Alpha Vantage Key**（可选，备用数据源）
...
- 部分数据源对限流敏感，尤其是 `yfinance`（已降级为最后备选，FMP 优先）
- 回测可信度取决于时点快照覆盖率
...
| 数据层 | `_fetch_with_providers()` 通用引擎，FMP 主源 + Alpha Vantage / yfinance 备选 |
